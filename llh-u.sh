...
REMOTE_UPDATE_URL="https://example.com/llh-u.sh" # ЗАГЛУШКА: Замените на актуальный URL репозитория
...
declare -A TEXT_EN=(
  ...
  ["user_uid_error"]="Script can only be run by user with UID=1000."
...
declare -A TEXT_RU=(
  ...
  ["user_uid_error"]="Скрипт может быть запущен только пользователем с UID=1000."
...
init_logging() {
  ...
  chmod 0750 "$LOGS_ROOT_DIR"
  chown $USER:sudo "$LOGS_ROOT_DIR"
...
change_language() {
  ...
  if [[ -n "$selected_lang" ]]; then
    ...
    whiptail --msgbox "$(get_text "language_changed_success")" 10 60 # TODO: add this string to TEXT_EN/RU
  else
    ...
    case "$choice" in
      1) # Управление пакетами (Вызов функции модуля)
        ...
        if [[ -v REGISTERED_MODULES["packages"] ]]; then
          log_main "ACTION" "User selected Package Management. Calling module_packages_main."
          # Вызываем функцию модуля через косвенное расширение
          "${REGISTERED_MODULES["packages"]}"
        else
          handle_error "MODULE_NOT_LOADED" "$(get_text "module_not_loaded"): packages" # TODO: add module_not_loaded to TEXT_EN/RU
        fi
        ;;
      2) # Управление службами (Пример для будущих модулей)
        log_main "ACTION" "User selected Services Management. (Not implemented yet)"
        whiptail --msgbox "Service management not implemented yet." 10 60 # TODO: localize
        ;;
      3) # Настройки сети (Пример для будущих модулей)
        log_main "ACTION" "User selected Network Settings. (Not implemented yet)"
        whiptail --msgbox "Network settings not implemented yet." 10 60 # TODO: localize
        ;;
...
if [[ "$CURRENT_SCRIPT_DIR" != "$WORK_DIR" ]]; then
  # Это первый запуск или запуск извне WORK_DIR - выполняем бутреппинг
  if [[ ! -d "$WORK_DIR" ]]; then
    mkdir -p "$WORK_DIR"
  fi
  ...
  find "$CURRENT_SCRIPT_DIR" -maxdepth 1 -name "module_*.sh" -print0 | while IFS= read -r -d $'\0' file; do
...
if ! cd "$WORK_DIR"; then
  whiptail --msgbox "$(get_text "work_dir_access_error"): $WORK_DIR" 10 60
  log_main "ERROR" "$(get_text "work_dir_access_error"): $WORK_DIR"
  exit 1
fi
...
# --- 9. Проверка и актуализация версии ---
check_for_updates

# --- 10. Загрузка модулей ---
log_main "INFO" "Loading modules..."

# Пример загрузки модуля: module_packages.sh
...
if ! source "module_packages.sh"; then
  handle_error "MODULE_LOAD_ERROR" "$(get_text "module_load_error"): module_packages.sh"
  # Если модуль не загрузился, возможно, стоит выйти или показать ошибку и продолжить
  # В данном случае handle_error уже возвращает, но для критических модулей можно сделать exit 1
  exit 1 # Пример: если модуль пакетов критичен, то выйти
fi
...
# if ! source "module_services.sh"; then handle_error "MODULE_LOAD_ERROR" "$(get_text "module_load_error"): module_services.sh"; fi
# if ! source "module_network.sh"; then handle_error "MODULE_LOAD_ERROR" "$(get_text "module_load_error"): module_network.sh"; fi
